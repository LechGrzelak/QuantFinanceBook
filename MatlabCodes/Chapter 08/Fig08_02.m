...
% Effect of kappa -- term structure

TMat = linspace(0.01,25,25);
kappaV = [0.1, 0.5, 1.0, 2.0];
argLegend = cell(4,1);
idy = 1;
IV = zeros(length(TMat),length(kappaV));
for i=1:length(kappaV)
    kappaTemp = kappaV(i);
    for idx = 1:1:length(TMat)
        Ttemp = TMat(idx);
        cf        = @(u) ChFHeston(u, Ttemp, kappaTemp,vBar,gamma,rho, v0, r);
        valCOS    = CallPutOptionPriceCOSMthd(cf,CP,S0,r,Ttemp,[S0],N,L);
        IV(idx,idy) = ImpliedVolatility(CP,valCOS,S0,Ttemp,S0,r,0.3)*100;
    end
    argLegend{i} = sprintf('kappa=%.2f',kappaTemp);
    idy = idy +1;
end
MakeFigure(TMat, IV,argLegend,'Implied ATM volatility for parameter \kappa')
xlabel('\tau-time to maturity')

% Effect of v0 -- term structure

v0V = [0.1, 0.3, 0.5, 1.0];
argLegend = cell(4,1);
idy = 1;
IV = zeros(length(TMat),length(v0V));
for i=1:length(v0V)
    v0Temp = kappaV(i);
    for idx = 1:1:length(TMat)
        Ttemp = TMat(idx);
        cf        = @(u) ChFHeston(u, Ttemp, kappa,vBar,gamma,rho, v0Temp, r);
        valCOS    = CallPutOptionPriceCOSMthd(cf,CP,S0,r,Ttemp,[S0],N,L);
        IV(idx,idy) = ImpliedVolatility(CP,valCOS,S0,Ttemp,S0,r,0.3)*100;
    end
    argLegend{i} = sprintf('v0=%.2f',v0Temp);
    idy = idy +1;
end
MakeFigure(TMat, IV,argLegend,'Implied ATM volatility for parameter v0')
xlabel('\tau-time to maturity')
...
for i=1:length(rhoV)
    ...
    argLegend{idx} = sprintf('vbar=%.2f',vBarTemp);
    idx = idx + 1;
end
MakeFigure(K, ivM,argLegend,'Effect of $\bar{v}$ on implied volatility')
