...
def GeneratePathsCGMY(NoOfPaths,T,r,S0,sigma,C,G,M,Y):
    ...
    N=1000
    pdf = COSDensity(varPhi,x,N,a,b)
    
    plt.figure(10)
    plt.plot(x,pdf)
    print('integral = {0}'.format(np.trapz(pdf,x)))   
    
    CGMY     = np.random.gamma(1.0,1.0,NoOfPaths)
    Z     = np.random.normal(0.0,1.0,NoOfPaths)
    theta = 0.5*(G-M)
    X_bar = theta * CGMY + sigma * np.sqrt(CGMY) * Z
    
    X     = (r+omega-0.5*sigma**2.0) * T  + X_bar    
    S     = S0 * np.exp(X)    
    return S    
...
def mainCalculation():
    ...
    cF_CGMY = CfCGMY(T,C,G,M,Y,sigma,r)
    ...
    # CGMY sampling via the inverse of the CDF
    y = np.linspace(0.01,150,2000)
    a = -5
    b = 5 
        
    # Here we use the fact that F_Y(y)=F_X(log(y))
    cF_CGMY = CfCGMY(T,C,G,M,Y,sigma,r,np.log(S0))
    F_Y= COSCdf(cF_CGMY,np.log(y),N,a,b)
    plt.figure(2)
    plt.plot(y,F_Y)
    plt.grid()
    plt.xlabel('y')
    plt.ylabel('CDF')
    plt.title('CDF- CGMY Model')
    ...
    plt.title('Call Opton Price for the CGMY Model, N=10000')
