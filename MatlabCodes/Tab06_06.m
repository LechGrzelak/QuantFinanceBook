function COS_CGMY_Density_Recovery_TABLE
close all;clc;
format short e
...
% First set Y value, and number of terms

%Ngrid = [32, 48, 64, 80, 96, 128];
%Y = 0.5;
    
% Second set Y valuem, and number of terms

Ngrid = [8,16, 24,32,40,48];
Y = 1.5;
...
for N = Ngrid
    ...
    error_txt{idx}=sprintf('error for N =%i is %.2e',N,error(idx));
    plot(x,f_X,'linewidth',1.5)
    legend_txt{idx} = sprintf('N=%i',N);
    time_txt{idx}= sprintf('time neeeded for evaluation = %2.2f miliseconds',timeEnd);
    idx = idx + 1;
end
legend(legend_txt)
error_txt
